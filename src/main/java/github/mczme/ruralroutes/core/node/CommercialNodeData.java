...
public record CommercialNodeData(
    ...
    /** 创建新节点数据 */
    public static CommercialNodeData create(UUID tradeNodeId, ResourceLocation themeName, Map<ResourceLocation, StockEntry> stocks, long timestamp) {
        return new CommercialNodeData(tradeNodeId, themeName, Map.copyOf(stocks), timestamp);
    }
    ...
    public CommercialNodeData withStock(ResourceLocation itemId, StockEntry newStock) {
        ...
        return new CommercialNodeData(tradeNodeId, themeName, Map.copyOf(newStocks), refreshTimestamp);
    }

    /** 更新刷新时间戳 */
    public CommercialNodeData withTimestamp(long newTimestamp) {
        return new CommercialNodeData(tradeNodeId, themeName, stocks, newTimestamp);
    }
}